...
import Styles from "../Css/Profissao.module.css";
...
function Profissao() {
  ...
  useEffect(() => {
    document.body.className = Styles.profissaoBody;
    return () => {
  ...
  const changePage = (page) => {
    if (page >= 0 && page < jobOptions.length) {
      setAnimationDirection(page > currentPage ? Styles.slideInRight : Styles.slideInLeft); // ✅ Define a direção
      setIsAnimating(true);
      setTimeout(() => {
        setCurrentPage(page);
  ...
  return (
    <div className={Styles.container1}>
      <div className={Styles.header}>
        <h1>Qual <span>Sua<br />Profissão?</span></h1>
        <p>MARQUE A OPÇÃO QUE MELHOR SE<br />ENCAIXA AO SEU <span>ATUAL EMPREGO</span></p>
      </div>
      
      <div className={Styles.buttons}>
        {currentPage > 0 && <button onClick={() => changePage(currentPage - 1)}>⟨</button>}
        {currentPage < jobOptions.length - 1 && <button onClick={() => changePage(currentPage + 1)}>⟩</button>}
      </div>

      <div key={animationKey} className={`${Styles.options} ${animationDirection}`}> 
...
//       setAnimationDirection(Styles.slideLeft);
//     } else if (direction === 'next' && currentPage < jobOptions.length - 1) {
//       setAnimationDirection(Styles.slideLeft);
//     }
...
//         changePage(currentPage + 1);
//       }
//       setIsAnimating(false); // Desativa após a animação
//     }, 500); // Tempo suficiente par
// };
...
//         {currentPage > 0 && <button  onClick={() => handleButtonClick('prev')}><svg width="42" height="44" viewBox="0 0 42 44" fill="none" xmlns="http://www.w3.org/2000/svg">
...
//       <div className={Styles.pagination}>{jobOptions.map((_, index) => (
//         <span
//           key={index}
//           className={`${currentPage === index ? Styles.active : ""}`}
//           onClick={() => changePage(index)}
